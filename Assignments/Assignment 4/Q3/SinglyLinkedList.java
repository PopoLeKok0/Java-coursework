...
public class SinglyLinkedList<E> implements List<E>, Iterable<E> {
    ...
    private int count(Node<E> currentHead, boolean found, E fromElement, E toElement) {
        if (currentHead==null) {
            return 0;
        } else if (found) {
            if (currentHead.value.equals(toElement)) {
                return 1;
            }
            return count(currentHead.next,true, fromElement, toElement)+1;
        } else {
            if (currentHead.value.equals(fromElement)) {
                return count(currentHead.next,true, fromElement, toElement)+1;
            }
            return count(currentHead.next,false, fromElement, toElement);
        }
    }
